...
There are exactly three sources of progress, with distinct roles:
- IMPLEMENTATION_PLAN.md: ordered phases and small work-unit IDs; what to do.
- BUILD_LOG.md, Current checkpoint: one short current pointer; where to continue.
...
1. Before the first edit in a unit: record scope, paths, baseline and next check.
2. After a coherent edit batch, before moving to another concern: mark code present but tests pending if appropriate.
3. Before a long test/build/install: record exact command and RUNNING/UNKNOWN, never PASS in advance.
4. After receiving its exit result: record outcome, environment, evidence location and next action.
5. At every verified commit and push; before a voluntary stop if execution remains available.
...
Keep this block at the top of BUILD_LOG, normally <=40 lines. Replace the pointer; append concise historical phase evidence below it.
...
- What exists now: concrete files/functions/screens, at most three bullets.
...
1. Read AGENTS, the docs index, this protocol and Current checkpoint. In a new context, read all normative specifications before production edits; in retained context re-read the current phase's topic owners. Do not ask the user to recount the chat.
...
6. Write a reconciled Current checkpoint with the next unit/action, then continue autonomously through remaining phase gates. Do not restart completed phases merely because conversation context was lost.
...
After each small verified implementation commit, update/commit the pointer as needed and normally push v1/mlforge if configured. No force, merge to main, or hidden branch switching. Record a push failure; local verification and remote backup are separate facts.
...
## User action
Use CODEX_PROMPT.md for either first start or continuation; open the same repository/checkout when local edits exist. In a retained conversation, a simple request to continue under this protocol suffices. No new product decisions or recap should be needed.
