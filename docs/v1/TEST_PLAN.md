...
P08 extends each installed `--training` Pilot journey through Try and Export: a keyboard-entered out-of-range value plus Missing inputs must show the runtime warnings, then Export builds `<task>_model` under the installed session's cwd. `verify_package.py` installs every such TUI-exported wheel into its own fresh virtual environment outside the checkout (index disabled, wheelhouse only, inherited `.pth` network guard with a denied control connection, pip check) and requires MLForge/Textual/Rich to be absent and the consumer `predict_many`/`transform_many` output to equal the in-app Try result within EXPORT_SPEC tolerance. This runs for both the wheel and sdist-derived applications and complements, not replaces, the six-model `export/test_install.py` matrix.

## P09 release tooling as implemented
`scripts/verify_release.py` composes the existing gates instead of re-implementing them: it requires the prepared wheelhouse (else prints the `--prepare-wheelhouse` instruction), runs `scripts/verify_package.py` (wheel and sdist-derived application installs, help/version, installed dataset and four-task Try/Export Pilot/PTY journeys, TUI-export consumer parity) and then the six-model `pytest -m install tests/mlforge/export/test_install.py` matrix with index access disabled, and writes commands, exit statuses, commit, cleanliness, versions and consumer evidence to `.mlforge-build/release-<python>.json`. CI runs it whenever consumer installations are enabled. `scripts/measure_peak_input.py` is the bounded peak-input guard (20,000 × 100 legal CSV under 20 MiB through load, two-model training, a 1,000-record prediction and export, with coordinator deadlines enforced and time/RSS recorded). `scripts/rehearse_recovery.py` is the P09 RESUME_PROTOCOL rehearsal on a disposable clone.
