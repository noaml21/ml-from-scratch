...
Application integration uses the current selected accepted handle and its original owned artifacts. An incomplete candidate set supplies a Boolean run-context flag to the export worker. The exporter copies that flag into wheel metadata/model card without rewriting the accepted bundle, fitting again, or changing schema/model bytes. Training metadata now records the actual prepared split policy, including stratified classification holdout. Parity probes retain supplied observed inputs and add all-missing plus genuinely unseen Category inputs where applicable; large probe lists are checked in runtime-sized batches. They are never bundled.

## P08 user-facing export
The Export screen shows the resolved default destination `cwd/exports`, `my_model` and `1.0.0` before creation, a one-time privacy/compatibility note, and field-specific errors (module/version syntax or reserved names, destination permission/collision). A known existing wheel is refused before a build is started; publication remains exclusive and still refuses a file that appears later. Success shows the literal published path, shell-quoted install commands for the exported Python minor, and a task-correct `predict`/`transform` example built from fitted field names and values (never source rows). Return to Results keeps the accepted selection; failure keeps it for retry.
