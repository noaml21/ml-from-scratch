...
P07 configuration screens render application descriptors/reviews and confirmed schema records. They invoke existing Service commands through the same Busy/help/resize deferral as dataset operations. Screen-local highlighted rows and last-presented review text are presentation state; authoritative choices and revision checks remain in application. An interrupted feature review redraws current selected IDs and requires a fresh review before accepting warnings for changed features. Preprocessing renders accepted policy JSON; it performs no splitting/fitting.
...
## P08 Try and export presentation
`Service.prediction_fields` returns immutable `InputField` records read from the selected accepted bundle's hash-verified `schema.json` (names, semantic kinds, fitted categories and numeric training range). It performs owned file I/O off the event loop and no fitting. The prediction worker converts the shared runtime's input-validation message into a field-scoped `PREDICT_INPUT` failure (record index, field name, fixed guidance) without echoing the value; other runtime errors keep their existing safe text. TUI Try and Export screens only collect literal text/`None` and present application snapshots. Export checks for an existing final wheel before building and records the same `EXPORT_EXISTS` failure; parent-side publication codes without a worker manifest use the exporter's wording. No-replace publication remains the authority.
