...
`preprocessing.prepare_run` freezes one shared row split and policy; it validates raw inputs statelessly through `prediction.runtime.normalize_record` and does not fit anything. Numeric/categorical matrix bounds are checked before allocating the normalized matrix. Supervised test size uses exact integer ceil(n/5); classification stratifies, both supervised goals shuffle with seed 42, and unsupervised goals keep all rows. Candidate fitting/evaluation consumes this plan without resplitting. `raw_records` preserves selected-field order and translates only canonical missing markers to None for the shared runtime.

## P04.2 implemented fitting
`training.fit_pipeline` is a child/headless service. It fits the standard preprocessing step once on prepared training rows, verifies transformed finite values/size/variation and clustering distinctness, then fits the standard estimator once. Both fitted objects remain in the same sklearn Pipeline used afterward; no custom transformer, second preprocessing fit or full-data refit is introduced. This ordering enforces degeneracy gates before estimator fitting while retaining train-only pipeline semantics. It reports safe warning categories and training-only all-missing fallback, rejects ConvergenceWarning, and explains zero-variance retained PCA components. The evaluated-bundle service below retains these fitted objects.

## P04.3.a implemented evaluation
`evaluation.py` computes finite metrics and immutable JSON diagnostics independently of fitting. It exposes the two primary table metric descriptors per task and deterministic ranking/recommendation over completed same-task candidates. Classification uses lexical training labels and a training-majority baseline; regression uses a training-mean baseline and explicit undefined R² reasons. Silhouette samples with a private seed-42 generator only above 2,000 rows; PCA reconstruction error is measured in standardized space. No displayed rounding affects these calculations. Training invokes these evaluators before constructing the retained bundle.

## P04.3.b implemented evaluated bundles
`training.train_candidate` fits once, evaluates the exact pipeline on the shared holdout (or all-row exploration), then saves and reload-validates that pipeline through the existing runtime. It returns an immutable CandidateResult with an opaque bundle handle; no estimator escapes into parent-facing state. Metrics retain nullable reasons; metadata includes safe fitting warnings and acknowledged review codes. Numeric ranges and category vocabularies come only from fitted rows. Storage, artifact-validation and candidate failures have safe distinct errors. Process exit/revision acceptance and partial-run labeling remain P05 responsibilities.
