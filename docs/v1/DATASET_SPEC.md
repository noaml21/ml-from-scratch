...
## P03.1 implementation notes
`datasets/importers.py` provides frozen format descriptors and `load_dataset`. Reads reject non-regular files before open, then compare device/inode through a nonblocking, no-follow descriptor. Byte limits are enforced again during reading; mutations detected during the read are rejected. CSV strict mode is supplemented by quote/field/column validation. JSON numbers retain lexical text through parse hooks; tokens overflowing finite float64 are structural errors, while finite integers above the precision limit remain raw for later Category interpretation. Source locations in parse errors are one-based physical lines; canonical row IDs are zero-based tuple positions. Fingerprints include parser version, format, headers and raw cell kinds/text, never the source path. Effective schema fingerprints are added by schema review.

## P03.2 implementation notes
`inference.infer_schema` examines every cell; `interpreted` is a pure scalar view and never edits raw cells. Profiles retain detected and effective types, stable lexical samples and effective distinct counts. Mixed native JSON kinds that already share a valid numeric/boolean interpretation are compatible; incompatible categorical/identifier mixtures carry MIXED_KINDS until explicit override. `validation.change_type` validates all present cells before returning a new revision; None resets to detection. `effective_fingerprint` combines canonical provenance with ordered effective types/override state, without a path. `datasets.records.preview` (also available from `validation`) includes exact totals, first-50 label, missing percentages and escaped controls. Optional display snippets leave raw records untouched; TUI uses literal cell-width truncation and exposes full selected values in help. `prepare.preparation_prompt` accepts only allowlisted diagnostics/formats, and `save_prompt` publishes an owner-only text file atomically without replacement.
