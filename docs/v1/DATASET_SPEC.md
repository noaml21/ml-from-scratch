...
`inference.infer_schema` examines every cell; `interpreted` is a pure scalar view and never edits raw cells. Profiles retain detected and effective types, stable lexical samples and effective distinct counts. Mixed native JSON kinds that already share a valid numeric/boolean interpretation are compatible; incompatible categorical/identifier mixtures carry MIXED_KINDS until explicit override. `validation.change_type` validates all present cells before returning a new revision; None resets to detection. `effective_fingerprint` combines canonical provenance with ordered effective types/override state, without a path. `preview` includes exact totals, first-50 label, missing percentages and escaped controls; TUI rendering must still disable markup. `prepare.preparation_prompt` accepts only allowlisted diagnostics/formats, and `save_prompt` publishes an owner-only text file atomically without replacement.
