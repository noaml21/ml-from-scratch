...
The package verifier exercises this dataset journey from fresh wheel and sdist-derived installations outside the checkout. Its Pilot and real terminal checks use installed services and children with inherited network guards, and preserve synthetic visual/terminal evidence. A separate training smoke mode also exercises all four goals through selection and inspection, with classification/PCA real PTYs, exact bundle retention and owned-resource cleanup. Try/export installed-package gates are P08.3; BUILD_STATE records whether the current candidate has passed these checks.

P07 configuration review and preprocessing preview run in the existing operation child. `Service.review_configuration` exposes existing task/target/feature policy and warnings as an immutable snapshot, initializes feature defaults once, and preserves intentional empty choices. `preview_preprocessing` calls the same preparation service used before training and accepts only a matching split/policy; it does not fit or start a training run. Goal, Target and Features now consume this review. Looks correct opens Goal after cancellable review; unsupervised tasks skip Target. Features keeps explicit checked state and warning acknowledgement. The preprocessing screen shows the actual prepared split/policy without fitting. Continue opens Models with both supervised models checked or the sole unsupervised model and bounded k/components input. Model choices and valid options remain in Service across Back/re-entry; invalid option text stays in the field while accepted state is unchanged. Train now invokes Service.train and shows actual preparation/training/evaluation phases, accepted candidate counts and safe failure reasons. Cancel waits for process cleanup, including a Back/quit request; completed candidates remain in the immutable run. Results and selected-model inspection now present accepted metrics and task diagnostics without refitting; the P07 phase gate has not passed.

Results follows completed training when a valid candidate exists, including retained cancelled results. Arrow keys move through the result table; ? explains the focused metric or full safe failure reason. Enter selects only a current successful candidate. Inspect details shows the existing confusion/per-class tables, regression residual/baseline summary, cluster sizes/centers or PCA variance/reconstruction. Selected model offers Try the model and Export package. Try asks the application for the selected bundle's fitted inputs (`Service.prediction_fields` reads its hash-verified schema) and renders one typed control per feature with a Missing toggle. Predict sends the literal values to `Service.predict`; the worker runs the shared runtime, and its input errors come back with the field name so the form marks that field. Export collects destination, module name and version, calls `Service.export` (private staging, verified build, no-replace publication) and then shows the created path, install commands and a task-correct Python example. Neither screen fits, normalizes or validates values itself.
