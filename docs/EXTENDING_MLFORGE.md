...
TUI shell changes belong in `tui/app.py`, `tui/screens/shell.py`, `tui/theme.tcss` and the single `tui/help.py` catalog. New dataset screens extend `tui/screens/dataset.py` and issue existing application commands. Preserve editable-letter keys, help focus restoration, minimum-size state, busy quit defaulting to Keep running, and actual PTY restoration. Package TCSS as a resource and retain lazy --help/--version imports.
...
Dataset UI changes also run through `scripts/installed_dataset_smoke.py`, invoked by `verify_package.py` for clean wheel and sdist-derived installations. Keep real Pilot/PTY coverage and inherited network guards; assertions in a source checkout alone do not verify installed resources or entrypoints. Check actual component selection colors as well as canonical palette contrast: native Textual defaults can introduce blended colors.

Configuration UI should use `Service.review_configuration`, its immutable `snapshot.review`, and `preview_preprocessing`. Add eligibility/warning rules in tasks.py and preparation rules in preprocessing.py, then test their real REVIEW/PREFLIGHT worker adapters. Keep full-table scans off the UI event loop; do not calculate defaults, warning detection or splits inside widgets. Review/prepared snapshots are invalidated by application configuration changes.

Configuration presentation lives in `tui/screens/configuration.py`: use the immutable reviewed choices and bounded profile records for labels/help. A highlighted Target row is never a chosen target. Keep the stable feature selection in Service; refresh checked markers after a failed review and retry review before acknowledging a changed selection. Models follows the prepared summary and uses Service.model_option_bounds/choose_models/choose_option; do not copy numeric-bound formulas into widgets. Add live Training through Service.train rather than fitting inside a screen.
