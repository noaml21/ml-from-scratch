...
Configuration presentation lives in `tui/screens/configuration.py`: use the immutable reviewed choices and bounded profile records for labels/help. A highlighted Target row is never a chosen target. Keep the stable feature selection in Service; refresh checked markers after a failed review and retry review before acknowledging a changed selection. Add Models/Training navigation after the prepared summary rather than fitting inside a screen.
