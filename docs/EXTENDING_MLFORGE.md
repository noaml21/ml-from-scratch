...
Read [AGENTS](../AGENTS.md) first. These are practical recipes for **later authorized changes**, not extra V1 requirements. The headless dataset, ML, prediction/export, process and application layers are implemented; the full-screen shell and initial manual-load route exist, with the remaining TUI journey in progress. BUILD_STATE records current verification rather than treating implementation as release completion. [ARCHITECTURE](v1/ARCHITECTURE.md) owns dependency/contracts; [PRODUCT_SPEC](v1/PRODUCT_SPEC.md) owns scope. Keep this guide synchronized with the actual code instead of maintaining parallel interfaces here.
