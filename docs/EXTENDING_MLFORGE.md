...
Dataset source choices use `Service.formats`, `Service.examples` and `load_example(filename)`; packaged examples go through ordinary worker-backed `load`. The lazy picker filters supported extensions and handles unreadable folders without recursive startup scanning. `datasets.records.visible_text` escapes control characters for literal presentation; validation retains its original import. Full first-50 preview and schema/Prepare controls are still pending P06.2.b/P06.3.
